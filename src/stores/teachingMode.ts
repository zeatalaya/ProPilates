...
interface TeachingModeState {
  ...
  isPlaying: boolean;
  elapsed: number; // seconds elapsed for current exercise
  totalElapsed: number; // total seconds elapsed in class
  ...
  progress: () => number; // 0-1 progress for current exercise
...
export const useTeachingModeStore = create<TeachingModeState>((set, get) => ({
  ...
  skipNext: () => {
    ...
    if (currentExerciseIndex < block.exercises.length - 1) {
      ...
      set({ isPlaying: false }); // class complete
    }
  },
  ...
  isComplete: () => {
    ...
    return (
      ...
      elapsed >= (lastBlock.exercises[lastBlock.exercises.length - 1]?.duration ?? 0)
    );
  },
}));
