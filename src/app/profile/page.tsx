    ...
    async function load() {
      ...
      if (xionAddress) {
        try {
          ...
          const uxion = data?.balances?.find(
            (b: any) => b.denom === "uxion",
          );
          setBalance(uxion ? (parseInt(uxion.amount) / 1_000_000).toFixed(2) : "0.00");
        } catch {
          setBalance("—");
        }
      }
        ...
        <Card>
          <CardHeader>
            <h3 className="flex items-center gap-2 font-semibold">
              <Zap size={16} className="text-violet-400" /> XION Balance
            </h3>
          </CardHeader>
          <CardBody>
            <div className="text-3xl font-bold">
              {balance ?? "—"}{" "}
              <span className="text-lg text-text-secondary">XION</span>
            </div>
            {xionAddress && (
